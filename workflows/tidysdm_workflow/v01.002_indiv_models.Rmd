...
```
Load in observations and background and do an initial split of the data into training and testing datasets.
```{r}
...
Using the training data from the previous step, complete a spatial cross validation using `spatial_block_cv()`. This allows us to divide up the area of interest into folds to address spatial correlation.
```{r}
...
Create an empty workflow: container that has 2 bits: recipe and a model. we have 7 predictors!
```{r}
wflow = workflows::workflow()
rec = recipes::recipe(head(aug_data), class ~ .)
```

Start modeling engine. vroom vroom. We choose parameters most important to tune for species distribution modeling based on the tidysdm workflow. With increased computing power we could potentially tune all parameters.
```{r}
model_maxent = maxent(
  mode = "classification",
  engine = "maxnet",
...
Set up a workflow set, which sets up a set of model and pre-processor combinations. If there were other pre-processors we wanted to implement, we could add them here to create new combinations.
```{r}
ws_models <-
  workflow_set(
    preproc = list(
...
Now we enter the tuning phase, where we apply the function `tune_grid()` to our grid of possible tuning parameter combinations. Because we are only tuning mtry in the random forest model which is based on our 7 covariates, there are only 7 possible parameter combinations and models. In maxent and brt, there are more than 20 combinations of tuning parameters, so there should be at least 20 models output from this function. 

Remember our 5 spatial cross folds from earlier? `workflow_map()` has 5 iterations of outputs from the 5 cross folds.
```{r}
ws_models <-
  ws_models |>
...
best_accuracy = show_best(rf_results, metric = "accuracy", n = n_metric(rf_results))
best_roc = show_best(rf_results, metric = "roc_auc", n = n_metric(rf_results))
best_boyce = show_best(rf_results, metric = "boyce_cont", n = n_metric(rf_results))
best_tss = show_best(rf_results, metric = "tss_max", n = n_metric(rf_results))

best_tbl = dplyr::bind_rows(best_accuracy, best_roc, best_boyce, best_tss)
best_tbl
model_group = best_tbl |>
  group_by(.config) |>
  mutate(avg_mean = mean(mean, na.rm = TRUE)) |>
...
```{r}
vi = variable_importance(x = final_rf_workflow, y = training(aug_split), type = "prob")
vi

vi_plot = ggplot() +
    geom_bar(data = vi, aes(x = 8, y = importance, fill = var),
