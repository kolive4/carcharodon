...
if (FALSE) {
  cfg = charlier::read_config("/mnt/ecocast/projects/koliveira/subprojects/carcharodon/workflows/tidy_md/m21.10096.yaml")
}
...
model_types = c("rf", "bt", "maxent", "gam", "glm")
preds = setNames(lapply(model_types, function(type){
  readr::read_csv(file.path(cfg$root_path, "tidy_workflow/versions", parsed_wf_v["major"], parsed_wf_v["minor"], cfg$t_wf_version, paste0(cfg$t_wf_version, "_", type, "_pred.csv")), show_col_types = FALSE)
}), model_types)

perdevex = lapply(preds, function(df) {
  perdev_ex(probs = df$.pred_presence, truth = df$class)
})

pdevs_tbl = enframe(perdevex, name = "model_type", value = "percent_deviance_explained") |>
  mutate(percent_deviance_explained = as.numeric(percent_deviance_explained))
...
all_metrics = dplyr::bind_rows(list(rf = rf_metrics, bt = bt_metrics, maxnet = maxnet_metrics, gam = gam_metrics, glm = glm_metrics),
                               .id = "model_type") |>
  left_join(pdevs_tbl, by = "model_type")
...
vi_plot = ggplot2::ggplot(data = all_vi) +
  ggplot2::geom_tile(ggplot2::aes(x = model_type, y = var, fill = importance)) +
  ggplot2::geom_text(ggplot2::aes(x = model_type, y = var, label = importance)) +
  ggplot2::scale_fill_gradient(low = "white", high = "orange", limits = c(0,100)) +
  ggplot2::labs(x = "Model Type", 
                y = "Covariate",
...
vi_fig_path = file.path(cfg$root_path,
  ...
  ggplot2::ggsave(vi_fig_path, plot = vi_plot, width = 8, height = 6, dpi = 300, create.dir = TRUE)
})
```
