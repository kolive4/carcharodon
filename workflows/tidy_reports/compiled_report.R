...
args = argparser::arg_parser("a tool to cast monthly predictions into one figure",
  ...
  argparser::add_argument(arg = "--config",
                          type = "character",
                          default = "/mnt/s1/projects/ecocast/projects/koliveira/subprojects/carcharodon/workflows/tidy_reports/c01.00000.01_12.yaml",
...
vi_plot = ggplot() +
  geom_bar(data = vi, aes(x = cfg$month, y = importance, fill = var),
           position = "fill", stat = "identity") +
  scale_x_continuous(breaks = seq(from = 1, to = 12, by = 1)) +
  theme_classic() +
