...
args = argparser::arg_parser("tidymodels/tidysdm modeling and forecasting for white shark habitat suitability",
                             name = "tidy_carcharodon.R",
                             hide.opts = TRUE) |>
  argparser::add_argument(arg = "--config",
                          type = "character",
                          default = "/mnt/s1/projects/ecocast/projects/koliveira/subprojects/carcharodon/workflows/tidy_workflow/t01.00000.01.yaml",
...
final_bt_predclass = bind_cols(
  predict(final_bt_workflow, testing(split)),
  predict(final_bt_workflow, testing(split), type = "prob")
)
...
depth = stars::read_stars(file.path(cfg$data_path, cfg$depth_path, sprintf("%s_%s_Bathy_depth_mon.tif", cfg$scenario, cfg$year))) |>
  dplyr::rename(log_depth = sprintf("%s_%s_Bathy_depth_mon.tif", cfg$scenario, cfg$year)) |>
...
rf_pred_plot
png(filename = file.path(vpath, sprintf("%s_%s_rf_plot.png", cfg$version, cfg$prediction)), 
    bg = "transparent", width = 11, height = 8.5, units = "in", res = 300)
print(rf_pred_plot)
dev.off()
...
bt_pred_plot
png(filename = file.path(vpath, sprintf("%s_%s_bt_plot.png", cfg$version, cfg$prediction)), 
    bg = "transparent", width = 11, height = 8.5, units = "in", res = 300)
print(bt_pred_plot)
dev.off()
...
maxent_pred_plot
png(filename = file.path(vpath, sprintf("%s_%s_maxent_plot.png", cfg$version, cfg$prediction)), 
    bg = "transparent", width = 11, height = 8.5, units = "in", res = 300)
